...
void PrHybridSeeding::findXProjections(unsigned int part, unsigned int iCase){
  ...
  Boundaries Bounds;
  //============Initialization
  std::array< HitIterat, NLAYERS> endsZone;
  std::array< HitIterat, NLAYERS> begZone;
  for(unsigned int i=0;i<NLAYERS;i++){
    //Always: first=0 last=1 middles=2,3 remains=4,5 (order of loops)
    InitializeBB(Bounds[i],number[i]);
  ...
  PrHit *fHit;
  std::array<bool,NLAYERS> first;  
  for( unsigned int j = 0; j<NLAYERS; j++){ first[j] = true; }
  
  //Used to cache the position to do a "look-around search"
  std::array<float,NLAYERS> xminPrev, xmaxPrev; 
